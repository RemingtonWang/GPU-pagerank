...
int main(int argc, char ** args) {
    ...
    if (argc != 4) {
	fprintf(stderr,"Wrong number of args. Provide input, output graph file and iterator number.\n");
        exit(-1);
    } 
...
/*************************************************************************/
    ...
    int * n_successors_h, *n_successors_d;            //每个顶点后继的个数(出度)
    ...
    while (fscanf(fp, "%u %u", &vertex_from, &vertex_to) != EOF) {
        ...
        if (vertex_to > n_vertices) {
            n_vertices = vertex_to;
	    }
	    n_edges++;
    }
    // std::cout << "from: " << vertex_from << ", to: " << vertex_to << ", num: " << n_vertices << std::endl;
    n_vertices++;
    
    std::cout << "num_v: " << n_vertices << ", num_e: " << n_edges <<std::endl;
...
/*************************************************************************/
    ...
    nvtxRangePushA("initializePagerankArray");
    initializePagerankArray<<<numOfBlocks,threadsPerBlock>>>(pagerank_d, n_vertices);
    nvtxRangePop();
    // nvtxRangePushA("setPagerankNextArray");
    ...
    while(epsilon < h_diff && iteration < n_iterations) {  //was 23
       ...
       // set the dangling value to 0 
        ...
        // nvtxRangePushA("addToNextPagerankArray");
        ...
        // std::cout << "diff: " << h_diff << std::endl; 

        // Make pagerank_d[i] = pagerank_next_d[i]
        // nvtxRangePushA("setPagerankArrayFromNext");
        setPagerankArrayFromNext<<<numOfBlocks,threadsPerBlock>>>(pagerank_d, pagerank_next_d, n_vertices);
        // nvtxRangePop();
        cudaDeviceSynchronize();

        clock_t cycles_iter = clock() - iter_start;
        // std::cout << "iter " << iteration << " time: " << cycles_iter * 1000 / CLOCKS_PER_SEC << std::endl;
        iteration++;
    }
